 ...
 * on {@link org.matsim.core.network.algorithms.NetworkSimplifier}. It goes through the network in two steps.
 ...
 *
 */
public final class NetworkSimplifier2 {

    private static final Logger log = Logger.getLogger(NetworkSimplifier2.class);
    ...
    private boolean generateShortLinkIds=false;
    private String delimiter = "--";

    private Set<String> simplifierIgnoreModes = new HashSet<String>();
    ...
    private Set<Id<Link>> ignorLinks = new HashSet<>();

    public void setIgnorLinks(Set<Id<Link>> ignorLinks) {
        this.ignorLinks = ignorLinks;
    }

    public void setGenerateShortLinkIds(boolean generateShortLinkIds) {
        this.generateShortLinkIds = generateShortLinkIds;
    }

    public void setSimplifierIgnoreModes(Set<String> simplifierIgnoreModes) {
        this.simplifierIgnoreModes = simplifierIgnoreModes;
    }
...
//							if(!outLink.getToNode().equals(inLink.getFromNode())){
                            if(  areLinksMergeable(inLink, outLink) ){
                                if(this.mergeLinksWithDifferentAttributes & !ignorLinks.contains(inLink.getId())
                                & !ignorLinks.contains(outLink.getId())){
...
//									inLink.getOrigId() + "-" + outLink.getOrigId(),
                                    ...
                                    if(bothLinksHaveSameLinkStats(inLink, outLink)){
                                        ...
                                        if(isHavingShortLinks){
                                            String[] inLinkIds = inLink.getId().toString().split(this.delimiter);
                                            String[] outLinkIds = outLink.getId().toString().split(this.delimiter);

                                            String inLinkStr = (inLinkIds.length >= 2) ? inLinkIds[0] :
                                                    inLink.getId().toString();
                                            String outLinkStr = (outLinkIds.length >= 2) ? outLinkIds[1] :
                                                    outLink.getId().toString();

                                            Link newLink;
                                            if (generateShortLinkIds) {
                                                newLink = NetworkUtils.createAndAddLink(network, Id.create(inLinkStr + this.delimiter + outLinkStr, Link.class), inLink.getFromNode(), outLink.getToNode(), inLink.getLength() + outLink.getLength(), inLink.getFreespeed(), inLink.getCapacity(), inLink.getNumberOfLanes(), NetworkUtils.getOrigId( inLink ) + "-" + NetworkUtils.getOrigId( outLink ), null);

                                            } else {
                                                newLink = NetworkUtils.createAndAddLink(network, Id.create(inLink.getId() + this.delimiter + outLink.getId(), Link.class), inLink.getFromNode(), outLink.getToNode(), inLink.getLength() + outLink.getLength(), inLink.getFreespeed(), inLink.getCapacity(), inLink.getNumberOfLanes(), NetworkUtils.getOrigId( inLink ) + "-" + NetworkUtils.getOrigId( outLink ), null);

                                            }

                                            newLink.setAllowedModes(inLink.getAllowedModes());

                                            String[] test = newLink.getId().toString().split(this.delimiter);
        ...
        List<Node> nList = new ArrayList<Node>(network.getNodes().values());
        for (Node node : nList) {
    ...
    private boolean bothLinksHaveSameLinkStats(Link linkA, Link linkB){
        ...
        // chsc
        for (String mode : this.simplifierIgnoreModes) {
            if (linkA.getAllowedModes().contains(mode) || linkB.getAllowedModes().contains(mode)) {
                bothLinksHaveSameLinkStats = false;
                break;
            }
        }

        if (ignorLinks.contains(linkA.getId()) || ignorLinks.contains(linkB.getId())) {
            bothLinksHaveSameLinkStats = false;
        }
    ...
    public static void main(String[] args) {
        ...
        org.matsim.core.network.algorithms.NetworkSimplifier nsimply = new org.matsim.core.network.algorithms.NetworkSimplifier();
